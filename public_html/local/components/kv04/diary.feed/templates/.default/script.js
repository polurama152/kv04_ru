/**
 * Лента дневника: весь её фронт.
 *
 * Жил внутри template.php и разросся до двух с лишним тысяч строк вперемешку
 * с разметкой. Отдельным файлом он проверяется обычным `node --check`,
 * кэшируется браузером отдельно от страницы и правится, не задевая шаблон.
 *
 * Всё, что знает только сервер, приходит data-атрибутами с #kv04-feed:
 * data-sessid, data-share-url, data-max-books. Ни одной PHP-вставки здесь
 * быть не должно — иначе файл снова перестанет быть просто скриптом.
 */
(function () {
	var root = document.getElementById('kv04-feed');
	if (!root) return;
	// Значения, которые знает только сервер, приезжают data-атрибутами на
