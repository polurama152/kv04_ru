...
**Пин меняется** в панели «Настройки» — но только при привязанной почте: без неё
забытый пин не вернуть ничем. Старый пин не спрашиваем (сессия им и открыта), зато
смена гасит все прочие устройства: метка `UF_PIN_CHANGED_AT` уезжает в сессию и cookie,
`Auth` сверяет её на каждом запросе. Забытый пин возвращает письмо: `?reset=<токен>` на
том же пути открывает форму нового пина и **внутрь не пускает** — утёкшее письмо не
читает записи тихо. Ответ на запрос письма всегда один, иначе по нему перечислялись бы
чужие адреса.

Дневник живёт на пути из опции `path` (на kv04.ru — `u`), у владельца может быть
...
`trash`, `shares`, `slugs` (адреса), `resets` (ссылки на возврат доступа, токен хэшем).
...
**Схема данных** — `Installer::SCHEMA_VERSION` (сейчас `11`). Поднимать при изменении
структуры HL или инфоблока: `ensure()` один раз переприменит её на каждом сервере.

**POST:** 19 действий в ленте, 6 на пин-паде — список в `processPost()` нужного
...
- `lib/` — `installer` (HL, инфоблок, pepper), `path` (путь, rewrite, переезды),
  ...
  (лестница блокировок), `resetservice` (возврат доступа по письму), `auth`
...
- **Письма — только `Mail\Event::sendImmediate()`.** Обычный `send()` кладёт строку в
  `b_event`, а разбирает очередь эпилог страницы: у дневника его нет (конфликт
  `main.broken.*`), AJAX и вовсе кончается `die()`, агенты на площадке не крутятся.
  Письмо просто не ушло бы, и молча.
